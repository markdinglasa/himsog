export enum MealPlanLineQuery {
  q001 = "SELECT mpl.Id, mpl.`IsBreakfast`, mpl.`IsLunch`, mpl.`IsSnack`, mpl.`IsDinner`, m.`Name` AS `MealName`, m.`Image` AS `MealImage`, SUM(nf.`Kilocalorie`) AS `MealKilocalorie` FROM `meal_plan_line` AS mpl LEFT JOIN `meal` AS m ON m.`Id` = mpl.`MealId` LEFT JOIN `nutrition_fact` AS nf ON nf.`MealId` = m.`Id` WHERE mpl.`MealPlanId` = ? GROUP BY mpl.Id, m.`Name`, m.`Image`, mpl.`IsBreakfast`, mpl.`IsLunch`, mpl.`IsSnack`, mpl.`IsDinner` ", // GET ALL
