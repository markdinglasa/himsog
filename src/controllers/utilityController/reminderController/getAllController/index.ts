...
export const ReminderGetAllController = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<any> => {
  try {
    const UserId: number = parseInt(req.params?.Id, 10); // ReminderId
    ...
    logging.log("----------------------------------------");
    logging.error("Reminder-Controller [GetAll]:", error.message);
    logging.log("----------------------------------------");
    return res
      .status(500)
