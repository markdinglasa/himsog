...
export const FeedbackGetAllController = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<any> => {
  try {
    const UserId: number = parseInt(req.params?.Id, 10); // FeedbackId
    ...
    logging.log("----------------------------------------");
    logging.error("Feedback-Controller [GetAll]:", error.message);
    logging.log("----------------------------------------");
    return res
      .status(500)
