...
import { RecipeQuery, DBTable, Error, Success } from "../../../../shared";
import { recipeValidator } from "../../../../validators";
import { UpdateService } from "../../../../services";
import { isFound } from "../../../../functions";

export const RecipeUpdateController = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<any> => {
  try {
    ...
    if (
      ...
      return res.status(401).json({ data: false, message: Error.m043 }); // check duplicate Recipe
    ...
    logging.log("----------------------------------------");
    logging.error("Recipe-Controller [Update]:", error.message);
    logging.log("----------------------------------------");
    return res
      .status(500)
