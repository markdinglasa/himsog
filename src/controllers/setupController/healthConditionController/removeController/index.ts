...
export const HealthConditionRemoveController = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<any> => {
  try {
    ...
    if (
      ...
      return res.status(401).json({ data: false, message: Error.m011 }); // check HealthCondition existence
    if (!(await RemoveService.byId(Id, DBTable.t004)))
      return res.status(401).json({ data: false, message: Error.m002 });
    return res.status(200).json({ data: true, message: Success.m003 });
  } catch (error: any) {
    logging.log("----------------------------------------");
    logging.error("HealthCondition-Controller [Remove]:", error.message);
    logging.log("----------------------------------------");
    return res
      .status(500)
