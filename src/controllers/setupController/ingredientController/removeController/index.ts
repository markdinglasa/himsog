...
export const IngredientRemoveController = async (
  ...
  try {
    ...
    if (!(await isFound(IngredientQuery.q002, ["Id"], [Number], [Id])).data)
      return res.status(401).json({ data: false, message: Error.m011 }); // check Ingredient existence
    if ((await isFound(IngredientQuery.q004, ["Id"], [Number], [Id])).data)
      return res.status(401).json({ data: false, message: Error.m020 }); // check Ingredient transactions or in used
