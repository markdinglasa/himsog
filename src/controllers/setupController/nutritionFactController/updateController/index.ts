...
export const NutritionFactUpdateController = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<any> => {
  try {
    const Id: number = parseInt(req.params?.Id, 10),
      Data: NutritionFactTable = req.body;
    ...
    if (
      (
        await isFound(
          NutritionFactQuery.q005,
          ["Id", "MealId", "Name"],
          [Number, Number, String],
          [Id, Data.MealId, Data.Name],
        )
      ).data
    )
    ...
    logging.log("----------------------------------------");
    logging.error("NutritionFact-Controller [Update]:", error.message);
    logging.log("----------------------------------------");
    return res
      .status(500)
