...
export const UserUpdateController = async (
  ...
  try {
    ...
    if (!Id || typeof Id !== "number")
      return res.status(200).json({ data: false, message: Error.m005 });
    if (
      !Data ||
      Data === undefined ||
      ...
      return res.status(200).json({ data: false, message: Error.m014 });
    ...
    if (!(await isFound(UserQuery.q006, ["Id"], [Number], [Id])).data)
      return res.status(200).json({ data: false, message: Error.m011 }); // check existence
    ...
    if (!(await UpdateService.record(Id, DBTable.t016, Fields, Types, Values)))
      return res.status(200).json({ data: false, message: Error.m003 });
