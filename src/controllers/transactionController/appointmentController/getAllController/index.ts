...
export const AppointmentGetAllController = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<any> => {
  try {
    const UserId: number = parseInt(req.params?.Id, 10); // AppointmentId
    ...
    logging.log("----------------------------------------");
    logging.error("Appointment-Controller [GetAll]:", error.message);
    logging.log("----------------------------------------");
    return res
      .status(500)
