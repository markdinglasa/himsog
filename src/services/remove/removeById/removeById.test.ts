...
describe('removeById', () => {
    ...
    it('should throw an error if Id is not a number', async () => {
        return await expect(removeById(parseInt('asd'), DBTable.t000)).rejects.toThrow('Id must be a valid number');
    });

    it('should throw an error if Id is missing or not provided', async () => {
        return await expect(removeById(undefined, DBTable.t000)).rejects.toThrow('Id must be a valid number');
    });

    it('should throw an error if Table name is not a string', async () => {
        return await expect(removeById(1, DBTable.t000)).rejects.toThrow('Table must be provided as a valid non-empty string');
    });
    ...
    it('should return rows if query and inputs are valid', async () => {
        ...
        const result = await removeById(1, DBTable.t001);
    ...
    it('should throw an error if pool connection fails', async () => {
        ...
        await expect(removeById(1, DBTable.t001)).rejects.toThrow('Connection failed');
    });
});
