...
export const SubscriptionReportPage: SFC = ({ ClassName }) => {
  ...
  const print = () => {
    ...
    if (element) {
      const options = {
        margin: 0.1,
        filename: `SubscriptionRevenue_${currentDate}.pdf`,
  ...
  const totalSales = (): number => {
    return (
      subsSales?.reduce((total: number, record: { Revenue: number }) => {
        return total + parseFloat(String(record.Revenue ?? 0));
      }, 0) ?? 0
    );
  };
  return (
            ...
            <S.Container className="w-full rounded-lg flex items-center justify-center ">
              <S.Content ref={pointerRef} className="w-full" id="content">
                <S.Divider className="flex w-full justify-between items-center mb-2">
                  <p className="text-sm">Report - Subscribers Per Month</p>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr>
                      ...
                      <th className="border p-1 font-medium text-md">Sales</th>
                    </tr>
                  </thead>
                  <tbody>
                    {subsSales?.length < 1 && (
                    ...
                    {subsSales &&
                      subsSales.map((record: any, index: number) => (
                        <tr key={index} className="text-center">
                          ...
                          <td className=" p-1 text-slate-800">
                            {formatNumber(Number(record?.Revenue ?? 0))}
                          </td>
                        </tr>
                      ))}
