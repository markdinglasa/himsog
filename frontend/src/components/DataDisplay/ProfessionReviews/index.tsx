...
import { ProfessionRatingTable, SFC } from "../../../types";
...
export const ProfessionRatingReviews: SFC = ({ ClassName }) => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = API.Setup.ProfessionRating.GetAll(Number(Id), page);
  ...
  const averageRating: number = useMemo(() => {
    if (reviews && reviews.length > 0) {
      const totalRating = reviews.reduce(
        (acc: number, review: ProfessionRatingTable) =>
          acc + parseFloat(review.Rate.toString()),
        0,
      );
          ...
          {isLoading ? (
            ...
            <>
              <div className="flex flex-col gap-[1rem]">
                {reviews.map((record: ProfessionRatingTable, index: number) => {
                  return (
                    <Fragment key={index}>
...
export default memo(ProfessionRatingReviews);
