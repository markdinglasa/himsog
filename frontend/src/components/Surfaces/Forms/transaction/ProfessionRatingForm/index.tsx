...
export const ProfessionRatingForm: SFC<FormProps> = ({
  ClassName,
  Title,
  OnClose,
  IsDisplay = false,
  ...
  const { Id: NutritionistId } = useParams<{ Id: string }>();
  const { add } = API.Setup.ProfessionRating.Add();
  const { auth } = useAuth();
  const { data, isLoading } = API.Setup.ProfessionRating.Get(
    Number(NutritionistId),
    auth?.user ?? 0,
  );
  const InitialValues: ProfessionRatingTable = {
    CreatedBy: data?.CreatedBy || (auth?.user ?? 0),
    UpdatedBy: data?.UpdatedBy || (auth?.user ?? 0),
    Remarks: data?.Remarks || null,
    UserId: data?.UserId || NutritionistId,
    Rate: data?.Rate || 0,
    IsHidden: Boolean(data?.IsHidden) || false,
  };

  const handleSubmit = async (values: ProfessionRatingTable) => {
    try {
      console.log(values);
      add(values);
      OnRefetch && OnRefetch();
        ...
        <S.Content className="content">
          ...
          <S.Divider>
            <Formik
              ...
              validationSchema={professionRatingValidator}
            >
              {({
                isValid,
                isSubmitting,
...
export default memo(ProfessionRatingForm);
